...
static bool parser_shutdown(struct parser *parser, struct lexeme *lexeme)
{
    ...
    if (!list_empty(&parser->stack)) {
        ...
        list_for_each_safe(&parser->stack, stacked, stacked_next, stack) {
            ...
            fprintf(stderr, "Parser error: EOF while looking for matching {{/%.*s}}", stacked->lexeme.value.value);
    ...
    if (parser->flags & FLAGS_NEGATE) {
        ...
        fprintf(stderr, "%s\n", "Parser error: unmatched negation");
        success = false;
    }
    if (parser->flags & FLAGS_QUOTE) {
...
action_start_iter:
    if (UNLIKELY(coro != NULL)) {
        ...
        fprintf(stderr, "%s\n", "Coroutine is not NULL when starting iteration");
        NEXT_ACTION();
    }
